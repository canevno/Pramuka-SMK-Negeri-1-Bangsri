                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $historySections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/60">
                            <h3 class="mb-3 text-sm font-semibold uppercase tracking-[0.12em] text-slate-600 dark:text-slate-300"><?php echo e($section['label']); ?></h3>

                            <div class="space-y-3">
                                <div>
                                    <label class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-200">Judul</label>
                                    <input type="text" name="<?php echo e($section['key']); ?>_title" value="<?php echo e(old($section['key'] . '_title', $settings[$section['key'] . '_title'] ?? '')); ?>" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="Judul bagian sejarah">
                                </div>

                                <div>
                                    <label class="mb-1 block text-xs font-medium text-slate-700 dark:text-slate-200">Konten</label>
                                    <textarea name="<?php echo e($section['key']); ?>_content" rows="6" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="Tulis isi sejarah..."><?php echo e(old($section['key'] . '_content', $settings[$section['key'] . '_content'] ?? '')); ?></textarea>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
