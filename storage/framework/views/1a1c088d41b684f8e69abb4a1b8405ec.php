...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($nextEvent): ?>
<div class="w-full">
        ...
        <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between md:text-left">
            ...
            <div class="order-1 flex-1 min-w-0 w-full md:w-auto">
                ...
                <h2 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-3xl text-center md:text-left mb-3 md:mb-4">
                    <?php echo e(is_array($nextEvent) ? ($nextEvent['title'] ?? 'Kegiatan') : ($nextEvent->title ?? 'Kegiatan')); ?>
                ...
                <div class="flex flex-col items-center gap-2.5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-center md:justify-start sm:gap-x-8 sm:gap-y-2">
                    ...
                    <div class="flex flex-col items-center gap-2.5 sm:flex-row sm:items-center"> 
                        ...
                        <div class="flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-300">
                            ...
                            <span class="font-medium"><?php echo e(is_array($nextEvent) ? ($nextEvent['date_formatted'] ?? '-') : ($nextEvent->date_formatted ?? '-')); ?></span>
                        </div>

                        
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(is_array($nextEvent) ? ! empty($nextEvent['guide_url']) : ! empty($nextEvent->guide_url)): ?>
                        <a href="<?php echo e(is_array($nextEvent) ? ($nextEvent['guide_url'] ?? '#') : ($nextEvent->guide_url ?? '#')); ?>" class="flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
            ...
            <div class="order-3 flex-shrink-0 flex items-center justify-center gap-4 md:flex-col md:items-center w-full md:w-auto">
                <div class="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 flex items-center justify-center flex-shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    ...
                    ?>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($eventLogo): ?>
                        <img src="<?php echo e(asset('storage/' . $eventLogo)); ?>" alt="Logo Kegiatan" class="w-full h-full object-cover" onerror="this.style.display='none'" />
                    <?php else: ?>
                ...
                <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 leading-tight font-extrabold md:hidden text-center">
                    <?php echo is_array($nextEvent) ? ($nextEvent['theme'] ?? '') : ($nextEvent->theme ?? ''); ?>
    ...
    <script>
        (function () {
            const target = new Date("<?php echo e(is_array($nextEvent) ? ($nextEvent['datetime_js'] ?? '') : ($nextEvent->datetime_js ?? '')); ?>");
...
<?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?><?php /**PATH C:\Users\Lenovo\Pramuka01\resources\views/sections/home/timeline.blade.php ENDPATH**/ ?>