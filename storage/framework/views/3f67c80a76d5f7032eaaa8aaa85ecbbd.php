...
<div class="space-y-4 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:space-y-6 sm:rounded-[2rem] sm:p-6 dark:border-slate-800 dark:bg-slate-900">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <h2 class="mt-2 text-xl font-bold text-slate-900 sm:text-2xl dark:text-white"><?php echo e($title ?? 'Kelola Pembina'); ?></h2>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 sm:text-sm"><?php echo e($description ?? 'Kelola data pembina dan penanggung jawab acara.'); ?></p>
        </div>
    ...
    <div class="space-y-2">
        <div class="flex items-center justify-end">
            <button type="button" id="toggle-pembina-list" class="hidden md:inline-flex items-center rounded-lg border border-indigo-200 bg-white px-2.5 py-1.5 text-[9px] font-semibold text-indigo-700 shadow-sm transition hover:bg-indigo-50 sm:text-[10px] dark:border-indigo-500/40 dark:bg-slate-900 dark:text-indigo-300 dark:hover:bg-slate-800" aria-expanded="false" aria-controls="pembina-list-card">
                Tampilkan daftar
            </button>
        </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <div class="flex-1 min-w-0">
                            <p class="font-medium text-slate-900 dark:text-white truncate"><?php echo e($pembina->name); ?></p>
                            <p class="text-xs text-slate-500 dark:text-slate-400 truncate"><?php echo e($pembina->jabatan); ?></p>
                        </div>
                        <span class="rounded-full px-2 py-0.5 text-[9px] font-semibold flex-shrink-0 <?php echo e($pembina->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200'); ?>">
                            <?php echo e($pembina->status ?: ($pembina->is_active ? 'Aktif' : 'Tidak aktif')); ?>

                        </span>
                    </div>
                    <p class="text-xs text-slate-600 dark:text-slate-400 mb-3 truncate"><?php echo e($pembina->phone ?: ($pembina->email ?: '-')); ?></p>
                    <div class="flex flex-col gap-1.5">
                        <form action="<?php echo e(route('admin.pembina.toggle', $pembina)); ?>" method="POST" class="w-full">
                            <?php echo csrf_field(); ?>
                        ...
                        <button type="button" onclick="document.getElementById('edit-pembina-<?php echo e($pembina->id); ?>').classList.toggle('hidden')" class="w-full rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1.5 text-[9px] font-semibold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-500/40 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">
                            Edit
                        </button>
                        <form action="<?php echo e(route('admin.pembina.delete', $pembina)); ?>" method="POST" onsubmit="return confirm('Hapus pembina ini?');" class="w-full">
                            <?php echo csrf_field(); ?>
                    ...
                    <div id="edit-pembina-<?php echo e($pembina->id); ?>" class="hidden mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                        <form action="<?php echo e(route('admin.pembina.update', $pembina)); ?>" method="POST" enctype="multipart/form-data" class="grid gap-2">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PUT'); ?>
                            <input type="text" name="name" value="<?php echo e(old('name', $pembina->name)); ?>" class="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nama lengkap" required>
                            <input type="text" name="jabatan" value="<?php echo e(old('jabatan', $pembina->jabatan)); ?>" class="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Jabatan" required>
                            <input type="text" name="phone" value="<?php echo e(old('phone', $pembina->phone)); ?>" class="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nomor telepon">
                            <button type="submit" class="rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-indigo-500 dark:bg-indigo-500 dark:hover:bg-indigo-400">
                                Simpan
                            </button>
                        </form>
                    </div>
        ...
        <div id="pembina-list-card" class="hidden md:block overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
            <div class="w-full overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-950">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $pembinas ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pembina): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            <td class="px-2 py-2.5 align-top font-medium text-slate-900 sm:px-4 sm:py-3 dark:text-white break-words"><?php echo e($pembina->name); ?></td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3 dark:text-slate-300 break-words"><?php echo e($pembina->jabatan); ?></td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3 dark:text-slate-300 break-words"><?php echo e($pembina->phone ?: ($pembina->email ?: '-')); ?></td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3">
                                <span class="inline-flex rounded-full px-2 py-1 text-[9px] font-semibold sm:text-[10px] <?php echo e($pembina->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200'); ?>">
                                    <?php echo e($pembina->status ?: ($pembina->is_active ? 'Aktif' : 'Tidak aktif')); ?>
                            ...
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3">
                                <div class="flex flex-col items-stretch gap-1.5 sm:flex-wrap sm:flex-row sm:items-center sm:gap-2">
                                    <form action="<?php echo e(route('admin.pembina.toggle', $pembina)); ?>" method="POST" class="w-full sm:w-auto">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <button type="button" onclick="document.getElementById('edit-pembina-<?php echo e($pembina->id); ?>').classList.toggle('hidden')" class="w-full rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1.5 text-[9px] font-semibold text-indigo-700 hover:bg-indigo-100 sm:w-auto sm:px-2.5 sm:py-1.5 sm:text-[10px] dark:border-indigo-500/40 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">
                                        Edit
                                    </button>

                                    <form action="<?php echo e(route('admin.pembina.delete', $pembina)); ?>" method="POST" onsubmit="return confirm('Hapus pembina ini?');" class="w-full sm:w-auto">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                        ...
                        <tr id="edit-pembina-<?php echo e($pembina->id); ?>" class="hidden bg-slate-50 dark:bg-slate-800/50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="<?php echo e(route('admin.pembina.update', $pembina)); ?>" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <input type="text" name="name" value="<?php echo e(old('name', $pembina->name)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nama lengkap" required>
                                    <input type="text" name="jabatan" value="<?php echo e(old('jabatan', $pembina->jabatan)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Jabatan" required>
                                    <input type="text" name="phone" value="<?php echo e(old('phone', $pembina->phone)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nomor telepon">
                                    <input type="email" name="email" value="<?php echo e(old('email', $pembina->email)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Email">
    ...
    <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/80 sm:p-5">
        ...
        <div class="block sm:block">
            ...
            <label class="block">
                ...
                <input type="text" name="phone" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none ring-0 transition focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-400" placeholder="Nomor telepon">
            </label>

            <label class="block">
                <span class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">Email</span>
                <input type="email" name="email" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none ring-0 transition focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-400" placeholder="email@example.com">
            </label>
