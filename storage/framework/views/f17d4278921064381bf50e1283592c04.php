<section class="w-full bg-white dark:bg-gray-950 py-12 md:py-20">
    <div class="max-w-7xl mx-auto px-4 md:px-8">
        
        <!-- Main Container -->
        <div class="flex flex-col lg:flex-row gap-8 lg:gap-10 lg:items-start">

            <!-- Left Section - Title & Description -->
            <div class="w-full lg:w-72 flex-shrink-0 text-center lg:text-left">
                <h2 class="text-4xl lg:text-5xl font-black text-gray-900 dark:text-white mb-4 font-['Poppins'] leading-tight tracking-tight">
                    Our Organitasion
                </h2>
                <p class="text-sm font-normal text-gray-600 dark:text-gray-400 leading-relaxed font-['Poppins']">
                    Kami merupakan Satuan Gugus Depan yang memiliki jumlah anggota yang besar dan struktur organisasi yang baik dan selalu menjunjung Tinggi Dasa Dharma Pramuka di Kehidupan Sehari - Hari
                </p>
            </div>

            <!-- Right Section - Cards Grid -->
            <div class="flex-1 min-w-0">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">

                    <!-- Card 1: Pembina -->
                    <div class="border-2 border-slate-300 dark:border-slate-500 bg-white dark:bg-gray-950 flex flex-col">
                        <div class="p-2">
                            <img src="<?php echo e(asset('images/visimisi/visimisi1.jpg')); ?>" alt="Pembina"
                                class="w-full aspect-video object-cover border-2 border-slate-300 dark:border-slate-500">
                        </div>
                        <div class="px-3 pb-3 pt-2 flex flex-col flex-1">
                            <h3 class="text-base lg:text-lg font-black text-gray-900 dark:text-white text-center mb-2 font-['Poppins']">
                                Pembina
                            </h3>
                            <p class="text-xs font-medium text-gray-600 dark:text-gray-400 text-center leading-snug font-['Poppins']">
                                Kami adalah pembina putra dan putri di ambalan KH. Acmad Fauzan Dan Dewi sartika. Pangkalan SMK Negeri 1 bangsri
                            </p>
                        </div>
                    </div>

                    <!-- Card 2: Dewan Ambalan -->
                    <div class="border-2 border-slate-300 dark:border-slate-500 bg-white dark:bg-gray-950 flex flex-col">
                        <div class="p-2">
                            <img src="<?php echo e(asset('images/visimisi/visimisi1.jpg')); ?>" alt="Dewan Ambalan"
                                class="w-full aspect-video object-cover border-2 border-slate-300 dark:border-slate-500">
                        </div>
                        <div class="px-3 pb-3 pt-2 flex flex-col flex-1">
                            <h3 class="text-base lg:text-lg font-black text-gray-900 dark:text-white text-center mb-2 font-['Poppins']">
                                Dewan Ambalan
                            </h3>
                            <p class="text-xs font-medium text-gray-600 dark:text-gray-400 text-center leading-snug font-['Poppins']">
                                Kami adalah pembina putra dan putri di ambalan KH. Acmad Fauzan Dan Dewi sartika. Pangkalan SMK Negeri 1 bangsri
                            </p>
                        </div>
                    </div>

                    <!-- Card 3: Anggota Dewan -->
                    <div class="border-2 border-slate-300 dark:border-slate-500 bg-white dark:bg-gray-950 flex flex-col">
                        <div class="p-2">
                            <img src="<?php echo e(asset('images/visimisi/visimisi1.jpg')); ?>" alt="Anggota Dewan"
                                class="w-full aspect-video object-cover border-2 border-slate-300 dark:border-slate-500">
                        </div>
                        <div class="px-3 pb-3 pt-2 flex flex-col flex-1">
                            <h3 class="text-base lg:text-lg font-black text-gray-900 dark:text-white text-center mb-2 font-['Poppins']">
                                Anggota Dewan
                            </h3>
                            <p class="text-xs font-medium text-gray-600 dark:text-gray-400 text-center leading-snug font-['Poppins']">
                                Kami adalah pembina putra dan putri di ambalan KH. Acmad Fauzan Dan Dewi sartika. Pangkalan SMK Negeri 1 bangsri
                            </p>
                        </div>
                    </div>

                    <!-- Card 4: Mitra -->
                    <div class="border-2 border-slate-300 dark:border-slate-500 bg-white dark:bg-gray-950 flex flex-col">
                        <div class="p-2">
                            <img src="<?php echo e(asset('images/visimisi/visimisi1.jpg')); ?>" alt="Mitra"
                                class="w-full aspect-video object-cover border-2 border-slate-300 dark:border-slate-500">
                        </div>
                        <div class="px-3 pb-3 pt-2 flex flex-col flex-1">
                            <h3 class="text-base lg:text-lg font-black text-gray-900 dark:text-white text-center mb-2 font-['Poppins']">
                                Mitra
                            </h3>
                            <p class="text-xs font-medium text-gray-600 dark:text-gray-400 text-center leading-snug font-['Poppins']">
                                Kami adalah pembina putra dan putri di ambalan KH. Acmad Fauzan Dan Dewi sartika. Pangkalan SMK Negeri 1 bangsri
                            </p>
                        </div>
                    </div>

                </div>
            </div>

        </div>
    </div>
</section><?php /**PATH C:\Users\Lenovo\Pramuka01\resources\views/sections/home/organisation.blade.php ENDPATH**/ ?>