...
<div class="space-y-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <h2 class="mt-2 text-2xl font-bold text-slate-900 dark:text-white"><?php echo e($title ?? 'Kelola Dewan Ambalan'); ?></h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400"><?php echo e($description ?? 'Kelola data dewan ambalan yang tampil di halaman depan.'); ?></p>
        </div>
            ...
            <table class="min-w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $members ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            <td class="px-4 py-3 font-medium text-slate-900"><?php echo e($member->name); ?></td>
                            <td class="px-4 py-3"><?php echo e($member->jabatan); ?></td>
                            <td class="px-4 py-3"><?php echo e($member->phone ?: ($member->email ?: '-')); ?></td>
                            <td class="px-4 py-3">
                                <span class="rounded-full px-2.5 py-1 text-[10px] font-semibold <?php echo e($member->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-700'); ?>">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center gap-2">
                                    <form action="<?php echo e(route('admin.dewan-ambalan.toggle', $member)); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <button type="button" onclick="document.getElementById('edit-dewan-ambalan-<?php echo e($member->id); ?>').classList.toggle('hidden')" class="rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[10px] font-semibold text-indigo-700 hover:bg-indigo-100">
                                        Edit
                                    </button>

                                    <form action="<?php echo e(route('admin.dewan-ambalan.delete', $member)); ?>" method="POST" onsubmit="return confirm('Hapus data dewan ambalan ini?');">
                                        <?php echo csrf_field(); ?>
                        ...
                        <tr id="edit-dewan-ambalan-<?php echo e($member->id); ?>" class="hidden bg-slate-50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="<?php echo e(route('admin.dewan-ambalan.update', $member)); ?>" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <input type="text" name="name" value="<?php echo e(old('name', $member->name)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Nama lengkap" required>
                                    <input type="text" name="jabatan" value="<?php echo e(old('jabatan', $member->jabatan)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Jabatan" required>
                                    <input type="text" name="phone" value="<?php echo e(old('phone', $member->phone)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Nomor telepon">
                                    <input type="email" name="email" value="<?php echo e(old('email', $member->email)); ?>" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Email">
                                    ...
                                    <input type="number" name="sort_order" value="<?php echo e(old('sort_order', $member->sort_order ?? 0)); ?>" min="0" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Urutan">
                                    <textarea name="bio" rows="3" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm md:col-span-2" placeholder="Deskripsi singkat"><?php echo e(old('bio', $member->bio)); ?></textarea>
