...
<?php $__env->startSection('content'); ?>
    <div class="space-y-6">
        ...
        <div class="rounded-[2rem] border border-slate-200 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 text-white shadow-[0_18px_45px_rgba(15,23,42,0.14)]">
            <div class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                ...
                <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur-sm">
                    ...
                    <p class="mt-2 text-base font-semibold text-slate-50"><?php echo e(count($latestAttendanceRecords ?? [])); ?> aktivitas</p>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between gap-2 border-b border-slate-100 pb-3">
                ...
                <a href="<?php echo e(route('admin.absensi')); ?>" class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100">
                    Lihat semua
                </a>
            </div>
            ...
            <div class="mt-4 space-y-2.5">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $latestAttendanceRecords ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    <div class="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-3 transition hover:border-slate-300 hover:bg-white">
                        <div class="flex items-center gap-3 overflow-hidden">
                            ...
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-slate-900">
                                    <?php echo e($record->kegiatan ?? 'Absensi Kegiatan'); ?>

                                </p>
                                <p class="text-[11px] text-slate-500">
                                    <?php echo e($record->created_at ? $record->created_at->translatedFormat('d M Y • H:i') : '-'); ?>
                        ...
                        <button type="button"
                                onclick="bukaModalAbsensi(<?php echo e($record->id); ?>)"
    ...
    <script>
    const API_BASE_URL = "<?php echo e(url('admin/absensi')); ?>";
