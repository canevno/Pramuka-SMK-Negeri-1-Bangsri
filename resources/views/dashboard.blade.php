...
<div class="space-y-6">
    <section class="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
            <a href="{{ route('admin.anggota') }}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-3 shadow-sm transition hover:border-emerald-200 hover:bg-white">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $usersCount }}</p>
            </a>
            <a href="{{ route('admin.news') }}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-3 shadow-sm transition hover:border-emerald-200 hover:bg-white">
                <div class="flex items-center justify-between gap-2">
                    <div class="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $newsCount }}</p>
            </a>
            <a href="{{ route('admin.gallery') }}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-3 shadow-sm transition hover:border-emerald-200 hover:bg-white">
                <div class="flex items-center justify-between gap-2">
                    <div class="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $galleryCount }}</p>
            </a>
            <a href="{{ route('admin.pendaftaran') }}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 shadow-sm transition hover:border-emerald-200 hover:bg-white">
                <div class="flex items-center justify-between gap-2">
                    <div class="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $registrationCount }}</p>
            </a>
            <a href="{{ route('admin.absensi') }}" class="group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 shadow-sm transition hover:border-emerald-200 hover:bg-white">
                <div class="flex items-center justify-between gap-2">
                    <div class="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $attendanceCount }}</p>
            </a>
        </div>
    </section>
            ...
            <div class="mt-4 space-y-3">
                @forelse($latestAttendanceRecords as $record)
                    <div class="flex gap-4">
                        ...
                        <div>
                            <p class="font-medium text-slate-950">{{ $record->participant_name }} — {{ $record->status }}</p>
                            <p class="mt-1 text-sm text-slate-500">{{ $record->participant_kelas }} • {{ $record->created_at->translatedFormat('d M Y H:i') }}</p>
                        </div>
                    </div>
                @empty
            ...
            <div class="mt-4 overflow-hidden rounded-[1.5rem] border border-slate-200">
                <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @foreach([['name'=>'Siti Aisyah','group'=>'Penggalang','date'=>'27 Mei 2025','status'=>'Menunggu','badge'=>'bg-amber-100 text-amber-700'],['name'=>'Andi Pratama','group'=>'Penegak','date'=>'27 Mei 2025','status'=>'Disetujui','badge'=>'bg-emerald-100 text-emerald-700'],['name'=>'Rizky Maulana','group'=>'Penggalang','date'=>'26 Mei 2025','status'=>'Menunggu','badge'=>'bg-amber-100 text-amber-700'],['name'=>'Dewi Lestari','group'=>'Penegak','date'=>'26 Mei 2025','status'=>'Ditolak','badge'=>'bg-rose-100 text-rose-700']] as $item)
                            <tr>
                                <td class="px-3 py-3">
                                    <div class="flex items-center gap-2">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">{{ strtoupper(substr($item['name'],0,1)) }}</div>
                                        <div>
                                            <p class="font-semibold text-slate-950">{{ $item['name'] }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-2 py-2 text-slate-600">{{ $item['group'] }}</td>
                                <td class="px-2 py-2 text-slate-600">{{ $item['date'] }}</td>
                                <td class="px-2 py-2">
                                    <span class="inline-flex rounded-full px-3 py-1 text-[10px] font-semibold {{ $item['badge'] }}">{{ $item['status'] }}</span>
                                </td>
                                <td class="px-2 py-2">
            ...
            <div class="mt-4 space-y-3">
                @foreach([['title'=>'Perkemahan Sabtu-Minggu Gugus Depan','date'=>'27 Mei 2025'],['title'=>'Pramuka SMKN 1 Bangsri Raih Juara Umum','date'=>'25 Mei 2025'],['title'=>'Latihan Rutin Ambalan Soedirman','date'=>'24 Mei 2025']] as $news)
                    <div class="flex gap-3 rounded-3xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div class="flex-1">
                            <p class="font-semibold text-slate-950">{{ $news['title'] }}</p>
