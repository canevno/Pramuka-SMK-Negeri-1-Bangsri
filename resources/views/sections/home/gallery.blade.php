...
<section class="py-10 md:py-16 bg-slate-50 dark:bg-gray-950 transition-colors duration-200">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="mb-6 border-b border-slate-200 pb-4 dark:border-slate-800 sm:flex sm:items-center sm:justify-between">
            <a href="{{ url('/galeri') }}" class="mt-3 hidden items-center justify-center gap-2 rounded-xl bg-[#0D1B2A] px-6 py-3 text-sm font-semibold text-white shadow-sm transition-all hover:bg-[#162b45] sm:mt-0 sm:flex">
        ...
        @if($galleryItems->isNotEmpty())
            <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:gap-4 lg:grid-cols-4">
                @foreach($galleryItems as $galleryItem)
                    @php $imageUrl = $resolveGalleryImage($galleryItem->image); @endphp
        ...
        <div class="mt-6 flex justify-center sm:hidden">
            <a href="{{ url('/galeri') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0D1B2A] px-6 py-3 text-sm font-semibold text-white shadow-sm transition-all hover:bg-[#162b45]">
                <span>Tampilkan Selengkapnya</span>
