...
<section class="space-y-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    @if(empty($stats) && empty($table) && empty($formFields))
        <div class="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/30 dark:text-slate-300">
            Halaman <strong class="dark:text-white">{{ $title }}</strong> sedang aktif dan siap dikembangkan sesuai kebutuhan modul.
        </div>
    @endif
</section>
@endsection