...
<div class="space-y-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['upcoming'] ?? 0 }}</p>
        ...
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['active'] ?? 0 }}</p>
        ...
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['completed'] ?? 0 }}</p>
                        ...
                        <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/50">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <button type="button" onclick="document.getElementById('edit-timeline-{{ $event->id }}').classList.toggle('hidden')" class="rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[10px] font-semibold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-500/40 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">
                                        Edit
                                    </button>
                        ...
                        <tr id="edit-timeline-{{ $event->id }}" class="hidden bg-slate-50 dark:bg-slate-800/50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="{{ route('admin.timeline.update', $event) }}" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <input type="date" name="date" value="{{ old('date', $event->date?->format('Y-m-d') ?? $event->date) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
                                    <input type="time" name="time" value="{{ old('time', $event->time) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
