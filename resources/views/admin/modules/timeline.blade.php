                        ...
                        <tr id="edit-timeline-{{ $event->id }}" class="hidden bg-slate-50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="{{ route('admin.timeline.update', $event) }}" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <textarea name="description" rows="3" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm md:col-span-2" placeholder="Deskripsi singkat kegiatan (maks 500 karakter)">{{ old('description', $event->description) }}</textarea>
