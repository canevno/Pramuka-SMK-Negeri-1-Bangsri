            ...
            <table class="min-w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @forelse($members ?? [] as $member)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <button type="button" onclick="document.getElementById('edit-dewan-ambalan-{{ $member->id }}').classList.toggle('hidden')" class="rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[10px] font-semibold text-indigo-700 hover:bg-indigo-100">
                                        Edit
                                    </button>
                        ...
                        <tr id="edit-dewan-ambalan-{{ $member->id }}" class="hidden bg-slate-50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="{{ route('admin.dewan-ambalan.update', $member) }}" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone', $member->phone) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Nomor telepon">
                                    <input type="email" name="email" value="{{ old('email', $member->email) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Email">
