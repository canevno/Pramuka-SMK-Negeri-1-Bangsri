...
<div class="space-y-4 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:space-y-6 sm:rounded-[2rem] sm:p-6 dark:border-slate-800 dark:bg-slate-900">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <h2 class="mt-2 text-xl font-bold text-slate-900 sm:text-2xl dark:text-white">{{ $title ?? 'Kelola Dewan Ambalan' }}</h2>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 sm:text-sm">{{ $description ?? 'Kelola data dewan ambalan yang tampil di halaman depan.' }}</p>
        </div>
            ...
            <table class="min-w-full text-left text-[11px] text-slate-600 sm:text-sm dark:text-slate-300">
                ...
                <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-950">
                    @forelse($members ?? [] as $member)
                        <tr>
                            <td class="px-2 py-2.5 align-top font-medium text-slate-900 sm:px-4 sm:py-3 dark:text-white">{{ $member->name }}</td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3 dark:text-slate-300">{{ $member->jabatan }}</td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3 dark:text-slate-300">{{ $member->phone ?: ($member->email ?: '-') }}</td>
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3">
                                <span class="inline-flex rounded-full px-2 py-1 text-[9px] font-semibold sm:text-[10px] {{ $member->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200' }}">
                            ...
                            <td class="px-2 py-2.5 align-top sm:px-4 sm:py-3">
                                <div class="flex flex-col items-stretch gap-1.5 sm:flex-wrap sm:flex-row sm:items-center sm:gap-2">
                                    <form action="{{ route('admin.dewan-ambalan.toggle', $member) }}" method="POST" class="w-full sm:w-auto">
                                        @csrf
                                    ...
                                    <button type="button" onclick="document.getElementById('edit-dewan-ambalan-{{ $member->id }}').classList.toggle('hidden')" class="w-full rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1.5 text-[9px] font-semibold text-indigo-700 hover:bg-indigo-100 sm:w-auto sm:px-2.5 sm:py-1.5 sm:text-[10px] dark:border-indigo-500/40 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20">
                                        Edit
                                    </button>

                                    <form action="{{ route('admin.dewan-ambalan.delete', $member) }}" method="POST" onsubmit="return confirm('Hapus data dewan ambalan ini?');" class="w-full sm:w-auto">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr id="edit-dewan-ambalan-{{ $member->id }}" class="hidden bg-slate-50 dark:bg-slate-800/50">
                            <td colspan="5" class="px-4 py-4">
                                <form action="{{ route('admin.dewan-ambalan.update', $member) }}" method="POST" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-2">
                                    @csrf
                                    @method('PUT')

                                    <input type="text" name="name" value="{{ old('name', $member->name) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nama lengkap" required>
                                    <input type="text" name="jabatan" value="{{ old('jabatan', $member->jabatan) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Jabatan" required>
                                    <input type="text" name="phone" value="{{ old('phone', $member->phone) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Nomor telepon">
                                    <input type="email" name="email" value="{{ old('email', $member->email) }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Email">
                                    ...
                                    <input type="number" name="sort_order" value="{{ old('sort_order', $member->sort_order ?? 0) }}" min="0" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" placeholder="Urutan">
                                    <textarea name="bio" rows="3" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 md:col-span-2" placeholder="Deskripsi singkat">{{ old('bio', $member->bio) }}</textarea>
