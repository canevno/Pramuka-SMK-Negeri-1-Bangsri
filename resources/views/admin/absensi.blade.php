    ...
    <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div class="grid gap-5 lg:grid-cols-4">
            <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                ...
                <p class="mt-4 text-3xl font-semibold text-slate-950">{{ number_format($totalCount ?? 0) }}</p>
            </div>
            <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                <p class="text-xs uppercase tracking-[0.18em] text-slate-500">Minggu ini</p>
                <p class="mt-4 text-3xl font-semibold text-slate-950">{{ number_format($weeklyCount ?? 0) }}</p>
            ...
            <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                ...
                <p class="mt-4 text-3xl font-semibold text-slate-950">{{ number_format($monthlyCount ?? 0) }}</p>
            ...
            <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                ...
                <p class="mt-4 text-3xl font-semibold text-slate-950">{{ number_format($yearlyCount ?? 0) }}</p>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2.5">
                                    <button type="button"
                                        ...
                                        class="js-btn-switch relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-emerald-500/50 {{ $item->is_active ? 'bg-emerald-500' : 'bg-slate-300' }}"
                                        role="switch"
                                        aria-checked="{{ $item->is_active ? 'true' : 'false' }}"
                                        title="Klik untuk {{ $item->is_active ? 'menonaktifkan' : 'mengaktifkan' }}">
                                        <span class="js-switch-thumb pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out {{ $item->is_active ? 'translate-x-5' : 'translate-x-0' }}"></span>
                                    </button>
                                    <span class="js-status-label text-xs font-semibold {{ $item->is_active ? 'text-emerald-600' : 'text-slate-400' }}">
                                        {{ $item->is_active ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-gray-500">
                                {{ $item->terakhir_melakukan ? \Carbon\Carbon::parse($item->terakhir_melakukan)->translatedFormat('d F Y H:i') : 'Belum Pernah' }}
                            </td>
                            <td class="px-4 py-3 text-gray-500">
                                {{ $item->rekam_count ?? 0 }}
                            </td>
    ...
    <div class="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        <div class="mt-4 flex items-center justify-end gap-2">
            ...
            @if(isset($absensi) && $absensi->id)
                <a href="{{ route('admin.absensi.exportWord', $absensi->id) }}" 
            ...
            <a href="{{ route('admin.absensi.export', $absensi->id ?? null) }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg">
                Export Excel
            </a>
...
<div id="modalTambahPetugas" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50 backdrop-blur-xs p-4">
    <div class="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-gray-200">
        ...
        <form method="POST" action="{{ route('admin.petugas.store') }}" class="space-y-3">
            @csrf
            <div>
